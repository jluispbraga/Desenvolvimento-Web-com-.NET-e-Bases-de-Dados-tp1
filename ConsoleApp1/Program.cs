...
class Program
{
    ...
    public static void Ex_10()
    {
        ...
        while (tentativas-- > 0)
        {
            Console.Write($"\nTentativas restantes: {tentativas + 1} | Seu palpite: ");
            ...
            Console.WriteLine(palpite < segredo ? "Muito baixo." : "Muito alto.");
        }

        Console.WriteLine($"Fim de jogo. O número era {segredo}.");
        Console.ReadKey();
    }
    
    // EXERCÍCIO 11
    public delegate double ConversorTemperatura(double temperatura);

    public static double CelsiusParaFahrenheit(double c) => (c * 9 / 5) + 32;
    public static double FahrenheitParaCelsius(double f) => (f - 32) * 5 / 9;
    ...
    public static void Ex_11()
    {
        ...
        ConversorTemperatura conversor = escolha switch
        {
        ...
        double resultado = conversor(valor);
    ...
    {
        Console.WriteLine("=== Filtro de Lista com Predicate ===");

        List<string> nomes = new() { "Ana", "Bruno", "Carlos", "Beatriz", "Amanda", "Arthur" };

        Console.Write("Filtrar nomes que começam com a letra: ");
        string letra = Console.ReadLine();

        Predicate<string> filtro = n => n.StartsWith(letra, StringComparison.OrdinalIgnoreCase);

        List<string> filtrados = nomes.FindAll(filtro);
        ...
        else
        {
            ...
            filtrados.ForEach(Console.WriteLine);
        }
